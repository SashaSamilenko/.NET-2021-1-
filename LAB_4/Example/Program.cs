...
namespace LAB_4
{
    class Program
    {
        ...
        {
            Console.WriteLine("Простая выборка элементов из d1");
            var q_1 = from x in d1
                      select x;
            foreach (var x in q_1)
                Console.WriteLine(x);

            Console.WriteLine("Простая выборка элементов из d2");
            var q_2 = from y in d2
                      select y;
            foreach (var y in q_2)
                Console.WriteLine(y);

            Console.WriteLine("Связи между d1 и d2");
            var l_1 = from z in lnk
                      select z;
            foreach(var z in l_1)
            {
                Console.WriteLine("id1 = "+z.d1+" id2 = "+z.d2);
            }
            /*
